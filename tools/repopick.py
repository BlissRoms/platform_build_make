...
if args.topic:
    url = 'http://review.blissroms.com/changes/?q=topic:%s' % args.topic
    if args.verbose:
...
for changeps in args.change_number:
    ...
    #   )]}'
    #   [ ... valid JSON ... ]
    url = 'http://review.blissroms.com/changes/?q={change}&o={query_revision}&o=CURRENT_COMMIT&pp=0'.format(change=change, query_revision=query_revision)
    if args.verbose:
